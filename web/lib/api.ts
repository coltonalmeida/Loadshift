...
export interface Forecast {
  generated_at: string;
  stale: boolean;
  now: Omit<NowData, "stale" | "generated_at">;
  hours: ForecastHour[];
}
...
export interface ModelCard {
  ...
  mef_method: { method: string; citation: string; mean_sum_slope: number; note: string };
}
...
export async function postSchedule(body: {
  appliance?: string; kwh?: number; watts?: number; duration_h: number;
